...
onready var car: KinematicBody2D = get_parent().get_node("Track/Car")
...
func _ready():
	...
	manage_action_thread.start(self, "manage_action")
...
#						err = semaphore.post()
						...
						if err != OK:
							print("Action manager error! :", err)
			else:
...
#		err = semaphore.wait()
		...
		if data != [] and data[0] == OK:
			if (data[1] as PoolByteArray) == ACTIONS["RUN"]:
				...
				message = "TURN_RIGHT"
			print("Received <- ", data[0], " ", data[1][0],data[1][1],data[1][2], \
					" ", (data[1] as PoolByteArray).get_string_from_ascii(), " ", message)
		...
		print("Sent ", _byte_array[0],_byte_array[1],_byte_array[2], " -> ", \
				_send_data(_byte_array), " ", _byte_array.get_string_from_ascii())
